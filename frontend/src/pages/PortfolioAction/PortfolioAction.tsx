...
import { NEWS_CREATE, NEWS_GET_ONE, NEWS_UPDATE, PORTFOLIO_CREATE, PORTFOLIO_GET_ONE, PORTFOLIO_UPDATE } from '@/utils/variables'
...
const NewsAction: React.FC = () => {
	...
	const { data } = useApi(`${PORTFOLIO_GET_ONE}/${id}`, { enabled: Boolean(id), suspense: true, keepPreviousData: false })
	const { mutate: mutateCreate, isLoading: createLoading } = useApiMutation(PORTFOLIO_CREATE)
	const { mutate: mutateUpdate, isLoading: updateLoading } = useApiMutationID("put", PORTFOLIO_UPDATE)
	...
	return (
		<Fragment>
			...
			<div className="page-head">
				<Button icon={<ArrowLeftOutlined />} onClick={() => navigate("/portfolio")}>Ortga qaytish</Button>
			</div>
			<div className="page">
